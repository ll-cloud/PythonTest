...
from common import config, logger, txt
import os
...
if __name__ == '__main__':
    config.get_config(os.path.join('../conf/conf.properties'))
    logger.debug(config.config)
    mail = Mail()
    mail.mail_info['filepaths'] = [os.path.join('../conf/module.html')]
    mail.mail_info['filenames'] = ['module.html']
    html = '第十二课'
    htmlModule = txt.Txt('../conf/' + config.config['mailtxt'])
